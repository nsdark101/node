...
class Stream {
  ...
  constructor(config, handle, session, direction) {
    ...
    this.#stats = new StreamStats(handle.stats);
    this.#state = new StreamState(handle.stats);
    const {
      onblocked,
      onerror,
      onreset,
...
class Session {
  ...
  openBidirectionalStream() {
    ...
    const stream = new Stream(this.#streamConfig, handle, this, 0);
  ...
  openUnidirectionalStream() {
    ...
    const stream = new Stream(this.#streamConfig, handle, this, 1);
    ArrayPrototypePush(this.#streams, stream);
    return stream;
  }
  ...
  sendDatagram(datagram) {
    ...
    return this.#handle.sendDatagram(new Uint8Array(datagram));
  }
  ...
  [kNewStream](handle, direction) {
    const stream = new Stream(this.#streamConfig, handle, this, direction);
    ArrayPrototypePush(this.#streams, stream);
...
class Endpoint {
  ...
  constructor(config, options = kEmptyObject) {
    ...
    validateFunction(onsession, 'config.onsession');
    validateObject(session, 'config.session');
    validateObject(stream, 'config.stream');
